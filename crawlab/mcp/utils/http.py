...
import crawlab.mcp
from crawlab.mcp.utils.constants import CRAWLAB_API_BASE_URL, CRAWLAB_USERNAME, CRAWLAB_PASSWORD
...
def api_request(
    ...
    if endpoint not in ['/login', '/system-info']:
        token = get_api_token()
        headers['Authorization'] = f"Bearer {token[:5]}...{token[-5:]}" if token else None
...
def get_api_token() -> str:
    ...
    if crawlab.mcp.utils.constants.CRAWLAB_API_TOKEN:
        logger.debug("Using cached API token")
        return crawlab.mcp.utils.constants.CRAWLAB_API_TOKEN
    ...
    try:
        response = requests.post(
            ...
            json={"username": CRAWLAB_USERNAME, "password": "********"},  # Mask password in logs
        )
        
        login_time = time.time() - start_time
        logger.info(f"Login request completed in {login_time:.2f} seconds with status code: {response.status_code}")
        ...
        if token := response_data.get('data'):
            ...
            crawlab.mcp.utils.constants.CRAWLAB_API_TOKEN = token
            return crawlab.mcp.utils.constants.CRAWLAB_API_TOKEN
        else:
